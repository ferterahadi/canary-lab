...
| **Spawn an agent CLI** (Codex/codex) | `runAgentProcess` + `buildClaudeAgenticArgs` (`agent-sessions/logic/agent-process.ts`) | re-implement spawn + tee + idle |
| Idle / liveness timeout | `startIdleTimer` (`agent-sessions/logic/agent-idle-timer.ts`) | inline `setInterval` + `lastOutputAt` |
| Recover Codex's answer from stream-json stdout | `recoverClaudeFinalText` (`agent-sessions/logic/agent-stream.ts`) | re-parse envelopes inline |
| Path of Codex's session JSONL | `claudeSessionLogPath` (`agent-sessions/logic/agent-session-log.ts`) | recompute `~/.Codex/projects/...` |
...
Codex agentic argv comes from `buildClaudeAgenticArgs`. Each caller passes only
...
- Adding a Codex argv → use the shared arg builder; don't re-list the flags.
