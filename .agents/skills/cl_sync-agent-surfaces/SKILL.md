...
| 3 | Codex skill (full run loop + authoring) | `agent-integrations/Codex/skills/canary-lab/SKILL.md` |
| 4 | Codex skill (identical body to #3; differs only in frontmatter — Codex-addressed `description` wording, and only the Codex skill carries `type: skill`. Neither skill passes `client_kind`; the bridge auto-detects it from the connection) | `agent-integrations/codex/skills/canary-lab/SKILL.md` |
...
rules; drop authoring/export detail. Do not paste the full Codex skill in.
...
- Heal-claim policy: denylist — only runner-spawned PTY agents (`Codex-pty`/
  `codex-pty`) get `claimSuppressed: true`; interactive Codex/Codex clients
...
| Copying the Codex skill verbatim into the plugin skill | Plugin skill balloons; the condensation was intentional |
| Copying the Codex skill file verbatim over the codex one | Bodies match, but the frontmatter differs (Codex-addressed description; `type: skill` is Codex-only) — a whole-file copy clobbers it |
