...
9. If it returns `failed`, report the terminal status using `result.counts.statusLine` and relevant failure summary.
   ...
   - If the result carries `specEdits` (a spec changed **after this run started** — can appear on `passed`, `failed`, or `needs_heal`), the run executed a copy of the suite taken at run start, so the edited spec was **not tested** and the verdict says nothing about it. Relay `result.specEdits.message` and follow `result.specEdits.nextSteps`: restore the spec to what the run started with, or ask the human to adopt: call get_test_review(runId), show the exact patch (read patchPath if needed), then call review_test_changes(runId, review_revision) for human elicitation. If available, reviewUrl opens the side-by-side viewer in the client browser panel. Unsupported clients use that page for adoption. Cancel/decline leaves work pending; changed revisions require a fresh review. No MCP tool can self-approve a spec edit, and a `passed` run with pending `specEdits` is a pass of the **original** suite — never report the edited tests as passed. `result.specEdits.hints` is the strength differential's reading, advisory only: a `kind: "weaker"` hint means an assertion was removed or loosened relative to what ran — restore it, a weaker assertion is never a repair; `kind: "cannot-classify"` means review by hand. Quote `specEdits.disclosure` with any hint you quote (one AI labelled, a second AI checked blind, no human).
