...
5. If `start_run` reports `already-claimed`, stop and tell the user which session owns the run. If it returns `claimSuppressed: true` (or `type: "claim_not_allowed"`), heal claiming is reserved for Claude/Codex **Desktop** clients — this CLI session cannot own the heal loop. The run still runs; do not call `wait_for_heal_task`. Report the run id and tell the user to drive heal from Claude/Codex Desktop or the web UI.
