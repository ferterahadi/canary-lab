...
1. Coverage needs a real source doc to ground on. If `get_feature_coverage` is **blocked** (`state.coverage: "blocked"`), read its `next:` field and follow it — don't present a menu. When `next` reports no source doc ("Setup needed", `sourceDocCount: 0`), **ask the user to attach or paste the PRD/spec in the chat** — never invent one or pull an external file. Drop source docs (specs, tickets, notes) into the feature with `write_feature_doc`, then generate the PRD summary: `start_coverage_job(feature, kind: "summary")` (async, single-flight — poll `get_coverage_job(jobId)` until `done`) or the synchronous `regenerate_prd_summary(feature)`. **Summary + Coverage are one exercise:** a summary job, on success, auto-chains the coverage engine and records the chained job as `chainedJobId` — poll that next to follow the full run. Re-run whenever the ledger's `state` reports the summary `stale` (`state.drift.changedDocs` names which docs moved) — requirement ids are preserved, so existing tags keep resolving. `list_feature_docs(feature)` shows what feeds the PRD; `write_feature_doc` adds a source doc and `delete_feature_doc(feature, relPath)` removes one; `clear_prd_summary(feature)` resets coverage to a blank slate — removes the generated summary **and** strips the `@req-*`/`@path-*` tags from the specs (other tags kept; an emptied tag list reverts the test to its pre-coverage shape), so a re-map starts clean instead of inheriting stale tags (the UI "Redo from the start" does the same — source docs are .txt/.pdf/.docx extracted to markdown on import).
2. Link each `test()` to its requirement with **Playwright tags on the test**: `test('…', { tag: ['@req-R3', '@path-happy'] }, …)` — `@req-<id>` (repeatable) and `@path-happy|sad|edge`. Tags are greppable and survive renames. (Legacy `// @requirement` / `// @path` comments above the test still parse as a migration fallback.) You can also run the engine to map for you: `start_coverage_job(feature, kind: "coverage")` infers which requirement each untagged test covers and writes the `@req-*` tag immediately — there is **no** accept/reject review gate. The engine writes the *tag* (mapping) — never the test body. Prefer to do the mapping YOURSELF (in this client) instead of a local agent? `start_external_coverage(feature)` spawns no agent — it returns the active requirements, the feature's tests (with file paths to read), and a `prompt`; read each test, then `submit_external_coverage(jobId, mappings)` and Canary writes the `@req-*` tags through its canonical tag-writer and recomputes the ledger (unknown ids/test names dropped). Needs a PRD summary first (else `status: "needs-summary"` — run a summary job first); shares the single-flight lock with `start_coverage_job`.
