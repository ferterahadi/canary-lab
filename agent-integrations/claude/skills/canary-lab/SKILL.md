...
scaffolding, environment capture, the test run, Parallel setup and Robustness Lab. Pass `stage_producer: "internal"`
...
requirement docs — request elicitation first; see the docs bullet below), **prd-summary** (distill the docs into testable
requirements), **specs↔coverage** (author the spec files, then map them
onto the requirements — authoring followed by mapping of changed or previously unexamined tests: the
...
- Boot verification overlaps docs and requirement preparation after scaffold. Tests
  wait for both. Only one checkpoint is answerable; pause and abort stop both owners.
- A docs collector also returns `{ requirements[], variantDimension? } in that
  handoff, as its prompt specifies. Canary pins this draft to the accepted docs
  and validates it in the summary stage; a separate summary handoff is needed
  only when the draft is missing, invalid, or stale.
- Mapping reuses previously examined test/requirement pairs, including unmappable
  answers, while their source, helpers, configuration and requirements stay
  unchanged. Account for every test in the supplied roster; do not add omitted
  tests. Coverage is still recomputed from real tags and test evidence.
...
- The server-owned stages (similarity, scaffold, env, the run's own Playwright
  execution, a `raw` export, Parallel setup and Robustness Lab) stay Canary's. Under an
...
surface it and end the foreground turn; Parallel setup and Robustness Lab do not need
...
| Status | Action |
| --- | --- |
| `running` + `links.evaluationZip` | Tell the user the Report is ready, explain Parallel setup and Robustness Lab continue in Canary's background, and end your turn. Do not poll merely to hold it open. |
...
Robustness Lab runs after Report and Parallel setup — server-conducted, never
parked. The green run's spec files re-run under the suite's perturbation envelope
(latency, duplicated writes, restarts) through a proxy on its declared port slots;
the stage's evidence is cells run, findings, confirmed 3/3 and skipped cells, and
`links.robustnessJobId` names the job. The existing Report stays downloadable;
refresh it after newer Lab findings settle if they should be included. It skips itself, with the reason on the
stage, when the run did not pass or no service takes an injected port. Read the
findings with `get_robustness(jobId)`; a confirmed finding is repaired outside the
flight with `start_run(feature, perturbation: finding.shrink.envelope)` (see
`canary-lab-run`) — never by editing the tests or the envelope; a skipped or
unconfirmed cell is never a pass.
