...
scaffolding, environment capture, and the test run. Pass `stage_producer: "internal"`
...
`submit_external_coverage`), **portify** (one park for the whole engagement:
drive the standalone loop — `submit_external_portify` on the given
`workflowId`, poll `get_portify`, re-edit on a failed verify — STOP at
`ready-to-save` and respond `submit` here with no data; never call
`save_portify`/`cancel_portify` yourself, the flight owns the save decision
and re-checks the workflow + overlay mark itself), **run/heal** (the flight
...
- The purely mechanical stages (similarity, scaffold, env, the run's own
  playwright execution — and a `raw` export) have no thinking in them and stay
  Canary's. Under an external flight the portify
  engagement and the heal loop are driven with the `canary-lab-portify` /
  `canary-lab-run` tools FROM the flight's own hand-off checkpoints.
