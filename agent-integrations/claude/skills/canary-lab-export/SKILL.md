...
5. A completed task also carries `certificate` (a digest) and `certificatePath`. The behavior certificate — `certificate.json` inside the archive, with the zero-dependency checker `verify-certificate.mjs` beside it — states which tests ran from which run-start suite snapshot (its digest), what each test asserted and how strongly, how the run ended, which live spec edits the verdict never executed, and the advisory hints — plus, when the Robustness lab ran against this run, a `robustness` block (the digest shows `robustness` as cells judged / confirmed / unconfirmed, or `none`): tests that passed green and failed under latency, a duplicated write or a restart, the smallest envelope that reproduces each, and the cells never judged. Relay `certificate.statement` and every line of `certificate.notProven` **as written**: it proves what ran from which snapshot, not the absence of weakening, and its counts are the harness's own (a never-run test is `not run`, never a pass; a `robustness: <n> cells not run` line means those cells were never judged; an unconfirmed finding is neither a defect nor a pass). Offer `certificate.verifyOffline` — the `node verify-certificate.mjs certificate.json --suite <dir>` command a third party runs to re-derive the hashes and assertion lines without Canary Lab. Only `download_evaluation_export` returns the full certificate inline; `get_evaluation_export` returns the digest plus the path.
