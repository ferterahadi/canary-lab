...
5. A completed task also carries `certificate` (a digest) and `certificatePath`. This internal behavior certificate is stored separately from the downloadable archive. Relay `certificate.statement` and every line of `certificate.notProven` as written: the recorded test copy, assertion strength, and run verdict have limits, including that absence of weakening is not proven. `get_evaluation_export` returns the digest and path; `download_evaluation_export` can return the full sidecar through MCP.
6. End with the exact `archivePath` and say that the ZIP contains `evaluation.html` and captured videos where available. Opening the report is the review step the export exists for.
