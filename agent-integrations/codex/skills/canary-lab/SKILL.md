...
1. Drop source docs (specs, tickets, notes) into the feature with `write_feature_doc`, then generate the PRD summary: `start_coverage_job(feature, kind: "summary")` (async, single-flight — poll `get_coverage_job(jobId)` until `done`) or the synchronous `regenerate_prd_summary(feature)`. Re-run whenever the ledger's `state` reports the summary `stale` (`state.drift.changedDocs` names which docs moved) — requirement ids are preserved, so existing tags keep resolving. `list_feature_docs(feature)` shows what feeds the PRD; `write_feature_doc` adds a source doc and `delete_feature_doc(feature, relPath)` removes one; `clear_prd_summary(feature)` removes the generated summary (the UI Docs-tab picker/pills do the same — .txt/.pdf/.docx are extracted to markdown on import).
2. Link each `test()` to its requirement with **Playwright tags on the test**: `test('…', { tag: ['@req-R3', '@path-happy'] }, …)` — `@req-<id>` (repeatable) and `@path-happy|sad|edge`. Tags are greppable and survive renames. (Legacy `// @requirement` / `// @path` comments above the test still parse as a migration fallback.) You can also run the engine to map for you: `start_coverage_job(feature, kind: "coverage")` infers which requirement each untagged test covers and writes the `@req-*` tag; with `reviewMode: true` it stores proposals (surfaced as `ledger.proposedMappings`) for `accept_coverage_mapping` / `reject_coverage_mapping` (by `testName`). The engine writes the *tag* (mapping) — never the test body.
