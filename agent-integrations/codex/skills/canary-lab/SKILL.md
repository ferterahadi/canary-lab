...
scaffolding, environment capture, the test run, Parallel setup. Pass `stage_producer: "internal"`
...
- The server-owned stages (similarity, scaffold, env, the run's own Playwright
  execution, a `raw` export, Parallel setup) stay Canary's. Under an
...
surface it and end the foreground turn; Parallel setup does not need
