...
3. Call `get_feature_coverage(feature)` for the ledger: per requirement → covering tests → last passing run + `gapType` (`untested` / `unverified` / `path-incomplete` / `verified` / `shallow-verified`) + coarse `coverageStatus` (covered/partial/uncovered), a grounded coverage % (verified ÷ total), rigor (`tierReached` / `tierAvailable` / `strictness` / `weakestAssertion` / `suggestedStrongerCheck`), `orphanTestNames` (tests with no requirement), and the derived `state` (summary × coverage axes + headline).
4. Act on gaps: `unverified` means the test exists but has no passing run (heal/run it); `path-incomplete` means a sad/edge path is missing; `shallow-verified` means it passes but only reaches a weak assertion tier (write a stronger check toward `suggestedStrongerCheck`, e.g. a browser confirming the real external effect rather than an app log); an orphan test needs a `@req-*` tag (or run the coverage engine). The UI's full-screen Coverage dialog shows the same data — both surfaces read the same computation.
