...
description: Use when exporting a Canary Lab run as an evaluation — "export the evaluation", "produce the report for run 7cvh", "give me the evaluation archive", "review what the run proved" — through the export MCP tools (start/submit_external_evaluation_export, get/list/download_evaluation_export). The run must be terminal (passed, failed, or aborted); this client writes the reasoning, Canary Lab renders the final evaluation.html + archive.
type: skill
---

# Canary Lab — Evaluation Export

Connect with the `export` MCP profile (`npx canary-lab mcp --profile export`);
the composite `lifecycle`/`full` profiles carry the same tools. The evaluation
export is the deliverable of a run: a self-contained archive (per-test
reasoning, verdicts, Playwright artifacts — video playback where the tests
drive a browser) the user reviews from `evaluation.html` without any Canary
Lab UI running.
...
- When `start_external_evaluation_export` no longer embeds the run snapshot, call `get_run` for the summary/failures while authoring.
