...
concurrently (benchmark arms / parallel runs). The
...
Canary runs automatically for declared injection, or on submit after edits — do not try to boot two copies yourself (those tools
...
- Use `review_portify` only when a standalone review needs a user decision; do not ask again when the user already authorized the action. Flight-owned reviews use `respond_flight_checkpoint` instead.
