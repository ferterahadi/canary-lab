...
4. if `start_run` returns an active run, continue it — but if it returns `type: "boot_session"` (or `executionType: "boot"`), the run is a held boot-only session with no tests and no heal task; do not claim heal or call `wait_for_heal_task`, just report services are up and that `abort_run` (confirm:true) stops them
5. `wait_for_heal_task` (if it ever returns `type: "boot_session"`, report services are up and stop — do not wait again)
