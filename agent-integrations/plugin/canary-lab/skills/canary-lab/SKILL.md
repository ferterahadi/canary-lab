...
If `start_run` says the active run is already claimed, stop and report the owning session instead of creating another run. Handle user interrupts explicitly: "pause", "intercept", or "pause and heal" means call `pause_run`; "stop heal" or "cancel repair" means call `cancel_heal`; "abort", "kill the run", or "stop everything" means call `abort_run` only with the required confirmation. Fix app/service code, not tests, unless the test is provably wrong. Keep the same `session_id` for the whole conversation. `heartbeat` is a low-level liveness refresh for long local repair stretches; `wait_for_heal_task`, `signal_run`, `write_journal`, and `get_heal_context` usually refresh liveness without an explicit heartbeat call.
