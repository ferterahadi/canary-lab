...
8. A healthy `/mcp/health` response also proves the tools are live. If a tool search looks empty, call `list_features` directly; searches of deferred tools omit tools already loaded in the session. Only an unknown-tool error means the server is disconnected. Ask the user to reconnect, then retry. Do not call `/mcp` through custom HTTP/JSON-RPC, including `curl`; only the health check above may use direct HTTP. A custom client bypasses client detection and discards the `next:` guidance in tool results, including recovery after a stage failure.
