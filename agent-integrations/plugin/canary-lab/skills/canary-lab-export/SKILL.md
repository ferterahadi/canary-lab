...
5. A completed task also carries `certificate` (a digest) and `certificatePath`. The behavior certificate — `certificate.json` inside the archive, with the zero-dependency checker `verify-certificate.mjs` beside it — states which tests ran from which run-start suite snapshot (its digest), what each test asserted and how strongly, how the run ended, which live spec edits the verdict never executed, and the advisory hints. Relay `certificate.statement` and every line of `certificate.notProven` **as written**: it proves what ran from which snapshot, not the absence of weakening, and its counts are the harness's own (a never-run test is `not run`, never a pass). Offer `certificate.verifyOffline` — the `node verify-certificate.mjs certificate.json --suite <dir>` command a third party runs to re-derive the hashes and assertion lines without Canary Lab. Only `download_evaluation_export` returns the full certificate inline; `get_evaluation_export` returns the digest plus the path.
