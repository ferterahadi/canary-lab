...
9. If it returns `failed`, report the terminal status using `result.counts.statusLine` and relevant failure summary.
   ...
   - If the result carries `specEdits` (test files changed **after this run started** — can appear on `passed`, `failed`, or `needs_heal`), the run used the tests recorded at run start, so the changed files were **not tested** and the result says nothing about them. Relay `result.specEdits.message` and follow `result.specEdits.nextSteps`: ask the human to adopt or restore by calling get_test_review(runId), showing the exact patch (read patchPath if needed), then calling review_test_changes(runId, review_revision) WITHOUT wait_for_decision for human elicitation in this agent session. For an active run, the human may adopt the exact revision and rerun that run, restore it, or leave it pending; after adoption continue with wait_for_heal_task and do not signal twice. For an ended passed/failed/aborted run, the human may approve the exact revision for a **new** run, restore it, or leave it pending; after approval call start_run **without** run_ref. Approval preserves the old result and is never a pass. Include helpers/fixtures/config, not only spec declarations. reviewUrl is optional side-by-side inspection, never a required operational step. If elicitation is unavailable, say that no question was presented; do not claim the human ignored it. Use a capable client, or only if the human chooses browser fallback, wait with the returned browser_wait_token and wait_for_decision:true. A Git commit, clean tree, restart, or chat statement is not a persisted review receipt. Cancel/decline leaves pending; changed revisions require a fresh review. No MCP tool can self-approve a test-file change, and a `passed` run with pending `specEdits` is a pass of the **recorded** tests — never report the changed tests as passed. `result.specEdits.hints` is the advisory check: a `kind: "weaker"` hint means an assertion was removed or loosened relative to the test that ran — restore it, a weaker assertion is never a repair; `kind: "cannot-classify"` means review by hand. Quote `specEdits.disclosure` with any hint you quote.
...
- During normal healing, omit `run_ref`/`force_new` so `start_run` continues the active run. `force_new` cannot replace an active run. Pending suite changes block fresh starts until `get_test_review` then `review_test_changes` records a human decision for the exact revision. An active run adopts and reruns; an ended run approves those bytes for a new run, which you start without `run_ref`. Restarting preserves recorded tests and review history but cannot turn an ended review into tested evidence.
