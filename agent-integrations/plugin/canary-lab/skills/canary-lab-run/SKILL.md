...
- When a blocked start returns `request_id`, carry it into `get_test_review` and `review_test_changes`. After the exact human decision, resume with `start_run(feature, request_id, session_id)` using the original session. Canary resumes internal requests itself; an external request stays with its original external client even when approval happens in the browser. Never create a replacement request or switch owners. `get_test_review` returns a read-only `browser_wait_token` for every client: use it with `wait_for_decision:true` to observe browser approval, including form-capable clients. After reconnect, get a fresh token and read the same durable request. A wait, transport notification, Git commit, or agent-written argument never approves changes.
