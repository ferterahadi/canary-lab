...
description: Use when adding a Playwright test, creating a feature, or working on tests in a canary-lab-enabled repo (one containing features/*/feature.config.cjs). Teaches the feature directory layout, config shape, envsets wiring, and test conventions so the agent doesn't reinvent or misplace files. Triggered by "add a Playwright test", "create a test in canary-lab", "new canary-lab feature", "set up a feature for X".
type: skill
---
