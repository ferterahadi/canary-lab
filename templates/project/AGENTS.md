...
2. If `sed` is also empty — instrument, then fix. `rg` to locate the handler, add `console.log` at decision points, write `.restart` with a `gather-evidence` hypothesis, and exit. On the next iteration, re-run `sed` and fix from real evidence. Remove the diagnostic logs in the same iteration that lands the fix.
