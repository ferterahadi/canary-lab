...
Follow `.codex/self-fixing-loop.md` (Phases 0–3) exactly, and consult
`AGENTS.md` for project context. Start by reading `logs/e2e-summary.json` for
the failure payload, along with `logs/diagnosis-journal.json` and
`logs/signal-history.json` if they exist — do not repeat a hypothesis that
already failed.
