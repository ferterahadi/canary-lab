...
Diagnose from the error messages first. Read a slice file only if the error alone isn't enough. `rg -n` distinctive literals in the repos listed. Fix the service/app code — never the test — then append a new iteration to `logs/diagnosis-journal.md` using this format:
...
Skip hypotheses already tried (check the journal). If the latest journal iteration's `outcome` is still `pending`, set it first based on the current index (`all_passed` / `partial` / `no_change` / `regression` — see `.codex/self-fixing-loop.md` for the rubric). If `heal-index.md` is missing entirely, fall back to `logs/e2e-summary.json` + `.codex/self-fixing-loop.md`.
