...
- **No full-file reads.** `rg -n` a distinctive literal from the service-log slice first, then read ±20 lines at the match (`sed -n 'N,Mp'` or equivalent).
...
1. **Preflight.** Read `logs/heal-index.md` — your single entry point. It lists every failure with its slice-file paths and summarizes the last 3 journal iterations.
   - If `heal-index.md` doesn't exist, fall back to `logs/e2e-summary.json`. If neither exists, tell the user the runner hasn't produced a summary yet and stop. If the index shows 0 failures, say "nothing to heal" and stop.
...
4. **Locate the bug.** Pick a distinctive literal from the inlined assertion or the slice (a quoted value, a camelCase/snake_case identifier, a numeric expected value). `rg -n` it inside the feature dir first, then the declared repo paths from `logs/manifest.json` (`repoPaths[]`). Read ±20 lines at the match.
5. **Edit.** Make the minimal fix. Append one entry to `logs/diagnosis-journal.json` with `feature`, `iteration`, `timestamp`, `failingTests`, `hypothesis`, `fix.file`, `fix.description`, `signal`.
...
If no slice files are listed for a failure in the index, or distinctive literals don't localize the bug, open `.codex/self-fixing-loop.md` for the fallback (Step 3: sed-by-slug on `logs/svc-*.log` + instrument-first).
