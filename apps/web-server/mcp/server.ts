...
- First Flight: start_flight(repoPaths, description) runs ONE background pipeline from bare repo(s) to a green, covered, healed run ending in an evaluation export (similarity → scout → scaffold → env → docs → PRD → specs↔coverage → portify → run → heal → export). The server conducts every stage and computes every verdict — you only approve checkpoints. Follow with get_flight and do what its next: field says: on waiting-for-approval call respond_flight_checkpoint(flightId) with choice (from checkpoint.options), values (missing-env KEY→value map), or data ({ configSource } for config-approval). At a prd-source checkpoint, FIRST distill any requirements from this conversation with write_feature_doc (dropped docs win the source hierarchy), then respond. One flight per repo set: re-calling start_flight follows an active one and resumes a paused one from its first open stage (fresh:true starts over). yolo:true skips every checkpoint except missing env secrets; on done, links.evaluationZip is the deliverable.
