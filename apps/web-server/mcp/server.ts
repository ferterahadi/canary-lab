...
- Flight: start_flight(repoPaths, description) runs ONE background pipeline from bare repo(s) to a green, covered, healed run ending in an evaluation export (similarity → scout → scaffold → env → docs → PRD → specs↔coverage → portify → run → heal → export). The server conducts every stage and computes every verdict — you only approve checkpoints. Follow with get_flight and do what its next: field says: on waiting-for-approval call respond_flight_checkpoint(flightId) with choice (from checkpoint.options), values (missing-env KEY→value map), or data ({ configSource } for config-approval — the feature is already scaffolded, so this writes through to its REAL on-disk feature.config.cjs; "redraft" re-runs the repo scan). Autopilot is ON by default: checkpoints with a safe default answer themselves (config-approval→approve, prd-source→continue when requirement docs already exist, coverage-stuck→accept-partial, portify-apply→apply, run-failed→export-as-is, export-mode→raw), each logged [autopilot] on its stage; the flight still parks on similarity-choice, missing-env, prd-source with NO docs yet, and any re-parked checkpoint. Start with autopilot:false to be asked at every checkpoint — do that when you plan to distill THIS conversation into requirement docs at the prd-source stop (write_feature_doc with content, or link_path for a local file, then respond "continue"). ONE flight record per feature: re-calling start_flight follows an active one and resumes a paused one from its first open stage; a settled one requires redo:true (restart from stage 1, discarding its stage evidence) or from_stage:"<stage>" (jump to a chosen stage — prerequisites are checked and a rejection names the missing artifact). A flight's repos and intent are FROZEN once it first starts: on redo / from_stage / resume OMIT repoPaths/description (the stored values are reused) — passing a DIFFERENT repo set or description is rejected with type:"flight_frozen"; to change them the user deletes the flight in the web UI (there is no delete tool or CLI flag). A flight with status:"paused", pauseReason:"queued" is waiting its turn behind another flight on the same repo(s) and auto-starts when that repo frees — narrate it as queued, not stuck, and do NOT ask the user to resume it (re-calling start_flight does start it early if they want). yolo:true skips every checkpoint except missing env secrets (export defaults to raw); on done, links.evaluationZip is the deliverable.`
