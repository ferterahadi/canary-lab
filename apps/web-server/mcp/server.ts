...
- Coverage is something YOU do (no local agent) in two handed-off steps, single-flight per feature, and it shows live in the GUI as an "external session" while you work. ① start_external_summary(feature) returns the source-doc paths, the previous requirement ids to PRESERVE, and a prompt; read the docs, extract requirements, submit_external_summary(jobId, requirements). ② start_external_coverage(feature) returns the requirements + the tests (with file paths) + a prompt; read each test, decide its requirement id(s), submit_external_coverage(jobId, mappings) — Canary writes the @req-* tags through its canonical tag-writer and recomputes the ledger (unknown ids/test names dropped). start_external_coverage needs a summary first (else status:"needs-summary" — run step ①). For a large PRD or many tests, OFFLOAD the reading/extraction to a background task or FAN IT OUT across docs/tests with subagents, then submit once. clear_prd_summary(feature) resets to a blank slate — removes the summary (+ coverage sidecars) and strips @req-*/@path-* tags from the specs (other tags kept).
