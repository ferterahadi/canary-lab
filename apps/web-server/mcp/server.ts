...
- Verified coverage: get_feature_coverage(feature) returns the grounded ledger — PRD requirements → annotated tests → last passing run + gap type (untested / unverified / path-incomplete / verified / shallow-verified), a coverage % (verified ÷ total, evidence-based math, never an opinion), and per-requirement rigor (tierReached/tierAvailable/strictness + weakestAssertion + suggestedStrongerCheck). Use it to find requirements with no passing run and tests that pass too laxly, then write a stronger/missing test (canary attests; it never writes the test for you). Annotate tests with @requirement <id> / @path happy|sad|edge comments above test(). list_feature_docs(feature) lists the docs that feed the PRD; after you add/edit docs call regenerate_prd_summary(feature) (preserves requirement ids).
