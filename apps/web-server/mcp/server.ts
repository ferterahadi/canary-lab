...
- Undo it: remove_portification(feature, confirm:true) reverts the feature config (the declared \`ports\` slots + \`\${port.x}\` health-check rewrites, restored from the snapshot saved with the overlay) and deletes the overlay, so the feature is no longer portified. Re-run start_portify to redo.`
