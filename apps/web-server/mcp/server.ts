...
- Semantic coverage: get_feature_coverage(feature) returns the ledger — PRD requirements → covering tests → gap type (untested / path-incomplete / covered), a coverage % (covered ÷ total — every declared path claimed by a mapped test) and a mapped % (requirements with ≥1 test), per-test strength (strong/solid/basic/shallow, graded from each test's assertion tiers), orphanTestNames (tests with no requirement), and a derived state view (summary × coverage axes + headline + drift naming changed docs). Coverage is DECOUPLED from test runs — it measures whether a test maps to each requirement+path, never whether a run passed. Link a test to a requirement with Playwright tags ON the test: test('…', { tag: ['@req-R3', '@path-happy'] }, …) (greppable, rename-proof; legacy @requirement/@path comments still parse as a fallback). Generation is ASYNC and Summary + Coverage are ONE exercise: start_coverage_job(feature, kind:"summary") regenerates the PRD summary AND auto-chains the coverage engine (recording the chained coverage job as chainedJobId); start_coverage_job(feature, kind:"coverage") re-runs only the engine. Both are non-blocking with a server-side single-flight guard; poll get_coverage_job(jobId) until done (for a summary job, then poll its chainedJobId), then re-read get_feature_coverage. The coverage engine infers which requirement each untagged test covers and writes the @req-* tag immediately — there is NO accept/reject review gate. Prefer to do the mapping YOURSELF (in this client) instead of a local agent? start_external_coverage(feature) spawns no agent — it returns the active requirements, the feature's tests (with file paths to read), and a prompt; read each test, decide its requirement id(s), then submit_external_coverage(jobId, mappings) and Canary writes the @req-* tags through its canonical tag-writer and recomputes the ledger (unknown ids/test names are dropped). It needs a PRD summary first (else returns status:"needs-summary" — run a summary job first) and shares the single-flight lock with start_coverage_job. Use the ledger to find untested/path-incomplete requirements and shallow tests, then write a stronger/missing test (canary maps the tag; it never writes the test body for you). list_feature_docs(feature) lists the docs that feed the PRD; write_feature_doc adds one and delete_feature_doc(feature, relPath) removes a source doc; after you add/edit/remove docs run a summary job (or regenerate_prd_summary(feature), preserving requirement ids). clear_prd_summary(feature) resets coverage to a blank slate — removes the generated summary (+ coverage sidecars) AND strips the @req-*/@path-* tags from the specs (other tags kept; an emptied tag list reverts the test to its pre-coverage shape), returning the feature to the no-summary state (returns untagged: the specs cleared).
...
- Make ports injectable (so a feature can boot concurrently — benchmark arms / parallel runs): start_portify(feature) kicks off an async workflow where a LOCAL agent rewrites every network listener to read an injected port, proven by a concurrent double-boot. Poll get_portify(workflowId) until status is "ready-to-save", then save_portify(workflowId, confirm:true) — or cancel_portify(workflowId, confirm:true). To adjust the result before saving, revise_portify(workflowId, feedback) resumes the agent and re-verifies (status cycles back through editing → verifying → ready-to-save; poll again). Feedback rounds are unbounded. One workflow at a time. Use list_portify_status to see which features are already portified vs still need it.
- Prefer to do the port rewrite YOURSELF (in this client) instead of a local agent? start_external_portify(feature) sets up the scratch worktree(s) and returns targets[] (edit paths) + the configPath + instructions; edit the listeners IN PLACE in those worktree paths and declare the matching \`ports\` slots in the config, then submit_external_portify(workflowId) verifies via the same double-boot. On "ready-to-save" call save_portify; if it returns to "editing", read verification.failureDetail, fix the worktree, and submit again. No local agent runs — the edits happen in your session. save/cancel/get/list_portify_status are shared with the local flow.
