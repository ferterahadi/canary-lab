...
- Make ports injectable (so a feature can boot concurrently — benchmark arms / parallel runs): start_portify(feature) kicks off an async workflow that rewrites every network listener to read an injected port, proven by a concurrent double-boot. Poll get_portify(workflowId) until status is "ready-to-commit", then commit_portify(workflowId, confirm:true) — or cancel_portify(workflowId, confirm:true). To adjust the result before committing, revise_portify(workflowId, feedback) resumes the agent and re-verifies (status cycles back through editing → verifying → ready-to-commit; poll again). Feedback rounds are unbounded. One workflow at a time. Use list_portify_status to see which features are already portified vs still need it.
- Committing lands the rewrite on a branch (canary/dynamic-ports-<feature>) — the user's checkout does NOT use it until merged. To finish, merge_portify(workflowId, confirm:true) merges that branch into each repo's currently checked-out branch; it refuses a dirty tree, aborts conflicts server-side (never leaves a repo mid-merge), reports per-repo results, and is safe to re-run (already-merged repos are skipped). If the user prefers, they can merge manually instead.`
