...
- name: {{featureJson}}, description: {{descriptionJson}}, envs: [{{envJson}}], featureDir: __dirname — emit these four EXACTLY as given. Do not reword the description or substitute your own prose: it is already correctly quoted, and a hand-written single-quoted string breaks the whole file on the first apostrophe (`stage's`, `doesn't`). Anything you want to say about the repo goes in a `//` comment.
...
- Declare a port slot whenever the port CAN be injected, and reference it via ${port.<slot>} in the healthCheck — in the http url AND in tcp.port (which takes the token string `'${port.<slot>}'`, not a bare number) — so concurrent runs don't clash.
...
- EXCEPTION — a service that HARDCODES its port with no env var and no flag: omit `ports` entirely and health-check the literal port it binds (`http://localhost:4300/`). Do NOT declare a slot it cannot read. A slot makes the harness allocate a free port and probe THAT, while the service still binds its hardcoded one — so boot verification fails on a service that would have started fine. The concurrency-prep stage runs later and is what makes such a port injectable; note the hardcoding in a `//` comment above the startCommand.
