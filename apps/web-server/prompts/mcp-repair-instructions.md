...
A needs_heal task may instead have context.serviceFailure: a required service failed after readiness. Read its kind, bounded excerpt and service log. The interrupted Playwright summary may be partial; do not count unrun tests as passed or failed. Fix service/app code, then signal_run kind:"restart" to request runner verification.
