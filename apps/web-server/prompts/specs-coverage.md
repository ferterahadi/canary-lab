...
- Keep test discovery independent of runtime prerequisites: load credential files, connect to services, and resolve required runtime artifacts inside hooks or fixtures, not at module scope. Missing prerequisites must still fail execution clearly; never replace them with empty defaults or conditional skips. `playwright test --list` must work before envsets are applied or services start.
- Read repository paths from feature configuration and honor `resolveRunRepoPath` for repo-owned files as well as processes. Do not introduce persistent references to temporary checkouts such as `/tmp` or `/private/tmp`; report an existing temporary-path dependency so its config and envset targets can be moved together.
