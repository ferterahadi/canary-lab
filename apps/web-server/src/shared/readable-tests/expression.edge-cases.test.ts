...
describe('readable expression edge cases', () => {
  ...
  it('renders calls whose result has one stable meaning', () => {
    const cases: Array<[string, string]> = [
      ...
      ["items.every((entry) => entry.status === 'READY')", "every item in items meets this condition: that item's status equals “READY”"],
      ['items.every((entry, index) => index > 0)', 'every item in items meets this condition: item index is greater than 0'],
      ["items.some((entry) => entry.tags.includes('urgent'))", "at least one item in items meets this condition: that item's tags contains “urgent”"],
      ["items.find((entry) => entry.status === 'READY')", "the first item in items where that item's status equals “READY”"],
      ...
      ['items.findLast(predicate)', 'the last item in items matching predicate'],
      ['items.findIndex(predicate)', 'the index of the first item in items matching predicate'],
      ['items.findLastIndex(predicate)', 'the index of the last item in items matching predicate'],
      ...
      ['items.some(function (entry) { return (entry as Row).id === targetId })', "at least one item in items meets this condition: that item's identifier equals targetId"],
      ['items.map((entry) => entry.id)', "items transformed so each item becomes that item's identifier"],
      ['items.map((entry, index) => entry.id + index)', "items transformed so each item becomes that item's identifier plus item index"],
      ['items.flatMap((entry) => entry.children)', "items transformed and flattened so each item becomes that item's children"],
      ['items.filter((entry) => entry.ready)', "items filtered to keep each item where that item's ready"],
      ['items.filter((entry, index, source) => entry.ready && index < source.length)', "items filtered to keep each item where that item's ready and (item index is less than collection's length)"],
      ["items.findIndex((entry) => entry.status === 'READY')", "the index of the first item in items where that item's status equals “READY”"],
      ["items.findLast((entry) => entry.status === 'READY')", "the last item in items where that item's status equals “READY”"],
      ["items.findLastIndex((entry) => entry.status === 'READY')", "the index of the last item in items where that item's status equals “READY”"],
      ...
      ['items.reduceRight((total, entry) => total + entry.amount, 0)', "the result of combining items from right to left, starting with 0 and updating the running value for each item to the running value plus that item's amount"],
      ['items.reduceRight((total, entry) => total + entry.amount)', "the result of combining items from right to left, starting with the last item and updating the running value for each remaining item to the running value plus that item's amount"],
      ...
      ['items.at(-1)', 'the item at index negative 1 in items'],
      ['items.flat()', 'items flattened by one level'],
      ['items.flat(1)', 'items flattened to 1 level'],
      ['items.flat(2)', 'items flattened to 2 levels'],
      ['items.entries()', 'the index-and-item pairs in items'],
      ['items.keys()', 'the indexes in items'],
      ['items.values()', 'the items in items'],
      ['items.indexOf(target)', 'the index of target in items'],
      ['items.lastIndexOf(target, 4)', 'the last index of target in items, searching from index 4'],
      ['items.includes(target, 2)', 'items contains target when searching from index 2'],
      ['items.copyWithin(0, 2, 4)', 'items after copying items from index 2 up to index 4 into index 0'],
      ['items.copyWithin(0, 2)', 'items after copying items from index 2 to the end into index 0'],
      ['items.fill(value)', 'items after filling items with value'],
      ['items.fill(value, 1, 3)', 'items after filling items from index 1 up to index 3 with value'],
      ['items.toReversed()', 'items in reverse order'],
      ['items.toSpliced(1, 2, replacement)', 'a copy of items with items changed from index 1, removing 2 items and inserting replacement'],
      ['items.toSpliced(1)', 'a copy of items with items changed from index 1'],
      ['items.with(2, replacement)', 'a copy of items with the item at index 2 replaced by replacement'],
      ['items.sort()', 'items sorted using default ordering'],
      ['items.toSorted((left, right) => left.rank - right.rank)', "items sorted by comparing left item's rank minus right item's rank"],
      ['items.slice()', 'a shallow copy of items'],
      ...
      ['Array.of(first, second)', 'a list containing first, second'],
      ['Array.of()', 'an empty list'],
      ['Array.from(values)', 'a list made from values'],
      ['Array.from(values, (value) => value.id)', "a list made from values, transforming each item to that item's identifier"],
      ['Array.fromAsync(values)', 'a list made from values, waiting for asynchronous values'],
      ['Array.fromAsync(values, (value) => value.id)', "a list made from values, transforming each item to that item's identifier and waiting for asynchronous values"],
  ...
  it('keeps effectful, dynamic, and unsupported expression shapes unresolved', () => {
    const cases: Array<[string, string]> = [
      ...
      ['items.at()', 'items.at()'],
      ['items.at(computeIndex())', 'items.at(computeIndex())'],
      ['items.flat(1, 2)', 'items.flat(1, 2)'],
      ['items.flat(computeDepth())', 'items.flat(computeDepth())'],
      ['items.entries(extra)', 'items.entries(extra)'],
      ['items.indexOf()', 'items.indexOf()'],
      ['items.indexOf(computeTarget())', 'items.indexOf(computeTarget())'],
      ['items.copyWithin(0)', 'items.copyWithin(0)'],
      ['items.copyWithin(0, computeStart())', 'items.copyWithin(0, computeStart())'],
      ['items.fill()', 'items.fill()'],
      ['items.fill(computeValue())', 'items.fill(computeValue())'],
      ['items.toReversed(extra)', 'items.toReversed(extra)'],
      ['items.with(1)', 'items.with(1)'],
      ['items.with(1, computeValue())', 'items.with(1, computeValue())'],
      ['items.toSpliced()', 'items.toSpliced()'],
      ['items.toSpliced(1, computeCount())', 'items.toSpliced(1, computeCount())'],
      ['Array.of(computeValue())', 'Array.of(computeValue())'],
      ['Array.from(computeValues())', 'Array.from(computeValues())'],
