...
describe('docs stage', () => {
  ...
  it('preserves a collector source conflict for focused elicitation without accepting a document', async () => {
    const document_resolution = { status: 'conflicting', searched: [repoDir], question: 'Which refund period applies?', candidates: [
      { label: 'Current', sources: [{ path: path.join(repoDir, 'current.md'), sha256: 'a'.repeat(64), reason: 'Spec says 30 days.' }] },
      { label: 'Draft', sources: [{ path: path.join(repoDir, 'draft.md'), sha256: 'b'.repeat(64), reason: 'Draft says 7 days.' }] },
    ] }
    const spawnAgent: FlightStageDeps['spawnAgent'] = async (opts) => {
      expect(opts.prompt).toContain('Discover documents before asking')
      expect(opts.prompt).toContain('inference requires')
      return { text: JSON.stringify({ document_resolution }) }
    }
    const adapter = docsStage(deps({ spawnAgent }))
    const { ctx, setStage } = ctxFor(manifest())
    const parked = await adapter.run(ctx)
    ...
    expect(outcome).toMatchObject({ kind: 'checkpoint', checkpoint: { kind: 'prd-source', data: {
      ...
      lastAttempt: { mode: 'collect-repo-docs', outcome: 'empty', reason: document_resolution.question },
    } } })
