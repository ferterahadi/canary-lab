...
import { launchEditorDir } from '../logic/editor-launch'
import { addWorktree, removeWorktree } from '../logic/runtime/repo-worktree'
import { listWorktrees } from '../logic/runtime/worktree-inventory'
import { loadProjectConfig } from '../logic/runtime/launcher/project-config'
import { loadFeatures } from '../logic/feature-loader'
import { getGitRoot } from '../logic/git-repo'
import type { BenchmarkStore } from '../logic/runtime/benchmark/store'
import type { SabotageSkill } from '../logic/runtime/benchmark/skills'
import type { BenchmarkManifest, StartBenchmarkInput } from '../logic/runtime/benchmark/types'

vi.mock('../logic/editor-launch', () => ({ launchEditorDir: vi.fn(() => 'vscode') }))
vi.mock('../logic/runtime/repo-worktree', () => ({ addWorktree: vi.fn(), removeWorktree: vi.fn(async () => {}) }))
vi.mock('../logic/runtime/worktree-inventory', () => ({ listWorktrees: vi.fn(async () => []) }))
vi.mock('../logic/feature-loader', () => ({ loadFeatures: vi.fn(() => []) }))
vi.mock('../logic/git-repo', async (orig) => ({
  ...(await orig<typeof import('../logic/git-repo')>()),
  getGitRoot: vi.fn().mockResolvedValue(null),
}))
vi.mock('../logic/runtime/launcher/project-config', () => ({ loadProjectConfig: vi.fn(() => ({ editor: 'cursor' })) }))
