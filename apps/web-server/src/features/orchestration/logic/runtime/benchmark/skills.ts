...
// The bundled sabotage skills ship with the webserver. dist layout puts this
// module at `dist/apps/web-server/lib/runtime/benchmark/` and the prompts at
// `dist/apps/web-server/prompts/`, so three levels up + `prompts` resolves in
// both `dist/` and `ts-node`/source (same relative shape).
