...
describe('linkFeatureDoc', () => {
  it('repairs a moved source under its old doc name without copying or resetting the summary', () => {
    const featureDir = writeFeatureConfig('relink')
    const target = path.join(tmpDir, 'before.md')
    const moved = path.join(tmpDir, 'after.md')
    fs.writeFileSync(target, 'v1')
    linkFeatureDoc(ctx(), { feature: 'relink', targetPath: target })
    const docsDir = path.join(featureDir, 'docs')
    const summary = path.join(docsDir, '_prd-summary.json')
    fs.writeFileSync(summary, 'existing baseline')
    fs.renameSync(target, moved)
    const res = linkFeatureDoc(ctx(), { feature: 'relink', targetPath: moved, relPath: 'before.md', relink: true })
    expect(res).toMatchObject({ ok: true, linked: true })
    expect(fs.readlinkSync(path.join(docsDir, 'before.md'))).toBe(moved)
    fs.writeFileSync(moved, 'v2')
    expect(fs.readFileSync(path.join(docsDir, 'before.md'), 'utf8')).toBe('v2')
    expect(fs.readFileSync(summary, 'utf8')).toBe('existing baseline')
    expect(fs.readdirSync(docsDir).sort()).toEqual(['_prd-summary.json', 'before.md'])
  })

  it.each(['missing', 'regular', 'working-link'])('refuses to relink a %s document', (kind) => {
    const featureDir = writeFeatureConfig('relink_guard')
    const docsDir = path.join(featureDir, 'docs')
    fs.mkdirSync(docsDir)
    const target = path.join(tmpDir, 'target.md')
    fs.writeFileSync(target, 'original')
    const dest = path.join(docsDir, 'prd.md')
    if (kind === 'regular') fs.writeFileSync(dest, 'existing')
    if (kind === 'working-link') fs.symlinkSync(target, dest)
    expect(linkFeatureDoc(ctx(), { feature: 'relink_guard', targetPath: target, relPath: 'prd.md', relink: true }))
  ...
  it.each([['an Error', new Error('EPERM')], ['a bare string', 'EPERM']])('keeps the broken link when symlink creation fails with %s, leaving no copy or staging residue', (_label, thrown) => {
    ...
    expect(fs.readdirSync(docsDir)).toEqual(['prd.md'])
  })
