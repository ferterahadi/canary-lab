...
describe('extractTraceSummary', () => {
  ...
  it('truncates an oversized per-request detail block instead of writing it in full', async () => {
    const traceZipPath = path.join(tmp, 'trace.zip')
    fs.writeFileSync(traceZipPath, 'zip')
    const outputDir = path.join(tmp, 'out')
    ...
    // > 16_384 bytes (REQUEST_DETAIL_MAX_BYTES) so capRequestDetail truncates.
    ...
    const network = fs.readFileSync(path.join(outputDir, 'network-failed.txt'), 'utf-8')
    expect(network).toContain('# Request 7 (headers + bodies)')
    expect(network).toContain('… (truncated,')
    expect(network).not.toContain(hugeBody)
  })
