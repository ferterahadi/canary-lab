...
export function registerFeatureAuthoringTools(ctx: ToolGroupContext): void {
  ...
  registerTool('get_feature_coverage', {
    description:
      'Get the Semantic Coverage Ledger: PRD requirements → mapped tests → gap type (untested / path-incomplete / covered) with a coverage % (covered ÷ total declared paths) and mapped % (requirements with ≥1 test), per-test strength (strong/solid/basic/shallow from assertion tiers), and docs-drift. coveragePct is claim-based — a tag claims the test→requirement+path mapping, regardless of run results. When the feature has a recorded run the ledger ALSO carries an additive proven axis (provenPct, totals.proven, per-requirement/path proven, provenRunId): covered = a tag claims it; proven = the covering test actually passed in the latest run. These fields are omitted when no run is recorded. The ledger ALSO carries the time axis (D11): per-requirement `enforcement` {state, provenAt, testsChangedAt, wordingChangedAt} where state is proven-unchanged (a green run over every mapped test is newer than both the tests\' and the wording\'s last change), tests-weakened (a mapped test was classified weaker AFTER the proof), proof-stale (a mapped test changed after the proof, or no run ever proved it), or wording-ahead (the wording moved after the tests and the proof); the ledger-level `enforcement` rolls it up as provenUnchanged/total for runId. Report these states — never a claim of your own — and fix a tests-weakened requirement by restoring the assertion, never by re-running. Use it to find untested/path-incomplete requirements and shallow tests. When the ledger is BLOCKED (state.coverage:"blocked") it carries a `next:` field with the recovery step; if it says no source doc exists, ASK THE USER to attach/paste the PRD in chat (never invent or pull one) before generating.',
