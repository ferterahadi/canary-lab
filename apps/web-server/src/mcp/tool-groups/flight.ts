...
export function registerFlightTools(ctx: ToolGroupContext): void {
  ...
  const flightNext = (view: Record<string, unknown>): string => {
    ...
    if (view.status === 'running') {
      if (reportReady) {
        return `${reportReady} Tell the user now, then end your turn. Parallel setup may continue as Canary-owned background work. Do not keep polling merely to hold this conversation open; the Flight page stays live, and re-check only when the user asks.`
      }
  ...
  registerTool('start_flight', {
    ...
    if (started.statusCode === 409 && startedBody.type === 'flight_exists_requires_choice') {
      return asJsonResult({
        ...
        next: 'This feature already has a flight record. Re-call start_flight with redo:true to restart from stage 1, or from_stage:"<stage>" to jump (prerequisites checked) — OMIT repoPaths/description so the frozen stored values are reused. Redo resets all stages; from_stage resets only affected artifacts. Tests & coverage keeps Parallel setup, Parallel setup resets itself. Completed Report downloads stay in history. A paused record resumes automatically without either flag — resume never wipes.',
      })
    }
