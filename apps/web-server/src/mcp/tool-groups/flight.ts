...
import { deriveFeatureSlug, flightReportNeedsRefresh } from '../../../../../shared/flights/types'
...
export function registerFlightTools(ctx: ToolGroupContext): void {
  ...
  const flightView = (raw: unknown): Record<string, unknown> => {
    ...
    return {
      ...
      ...(flightReportNeedsRefresh(raw as FlightManifest) ? { reportRefreshAvailable: true } : {}),
  ...
  const flightNext = (view: Record<string, unknown>): string => {
    ...
    const reportPath = links?.evaluationZip
    const reportReady = reportPath
      ? `The Report is ready at links.evaluationZip (${reportPath}) and can be reviewed now.`
      : ''
    const refreshNote = view.reportRefreshAvailable === true
      ? ' Newer Robustness Lab findings are available. The current Report stays downloadable; offer a refreshed export from_stage:"evaluation-export" if the user wants them included.'
      : ''
    if (view.status === 'waiting-for-approval') {
      ...
      if (cp?.kind === 'external-work') {
        ...
        if (data?.takeoverRequestedAt) {
          const reportHandoff = data.stage === 'portify' && reportReady
            ? ` ${reportReady} Once you release this legacy Parallel setup hand-off, tell the user the Report path and end your turn. Canary will continue Parallel setup as server-owned background work; do not poll merely to hold this conversation open.`
            : ''
          return `TAKEOVER REQUESTED by the user at ${data.takeoverRequestedAt}. Stop work on this step now, including any subagents or processes you started. Do not submit your result. Release the step with respond_flight_checkpoint(flightId, choice:"run-internally"); only that acknowledgement lets Canary start its local agent. If the user forces takeover before you acknowledge, your later checkpoint response will be rejected. Files you already wrote stay on disk, so tell the user what changed before releasing if they need to review it.${reportHandoff}`
        }
        if (data?.stage === 'portify' && reportReady) {
          return `${reportReady} This is a legacy client-owned Parallel setup hand-off from a Flight started before Parallel setup became server-owned. Do not perform the Portify work in this conversation. Release it now with respond_flight_checkpoint(flightId, choice:"run-internally"). Once released, tell the user the Report path and end your turn. Canary continues Parallel setup as persistent background work; do not poll merely to hold this conversation open.`
        }
      ...
      if (cp?.kind === 'portify-gate') {
        return `${reportReady ? `${reportReady} ` : ''}${base} This is the Parallel setup ask, after the Report but before any Portify agent or double-boot cost: "run" starts the server-owned background workflow (agent edits port wiring in a throwaway worktree, concurrent double-boot verifies — heavy stacks can take 30-60+ min; a sibling feature's saved overlay for the same app is reused and verified first, so the agent may not run at all); "skip" keeps the feature serial (runs go one at a time) and the flight continues — a later flight can ask again. The Report is independent of this choice and remains reviewable.`
      }
      if (cp?.kind === 'portify-apply') {
        return `${reportReady ? `${reportReady} ` : ''}${base} The background diff passed a concurrent double-boot. "apply" SAVES it as the feature's overlay (nothing lands in the product repos — runs apply it into throwaway per-run worktrees); "revise" REQUIRES feedback:"<what to change>" and sends Canary's agent back for another edit + re-verify pass (the checkpoint re-parks with the new diff); "cancel" discards the edits and SKIPS the stage — the flight continues WITHOUT Parallel setup (the feature stays serial; a later flight can retry). The Report remains valid regardless of this choice.`
      }
      return base
    }
    if (view.status === 'running') {
      if (reportReady) {
        return `${reportReady}${refreshNote} Tell the user now, then end your turn. Parallel setup and Robustness Lab are continuing as Canary-owned background work. Do not keep polling merely to hold this conversation open; the Flight page stays live, and re-check only when the user asks.`
      }
      return 'Flight is running — re-call get_flight to follow it; it parks on checkpoints and settles to done/paused/failed.'
    }
    if (view.status === 'paused' && view.pauseReason === 'queued') return `${reportReady ? `${reportReady} ` : ''}Flight is queued — it is waiting its turn behind another flight on the same repo(s) and starts automatically when that repo frees. No action needed; tell the user it is queued, not stuck. Only if they want it started early, re-call start_flight (it resumes a queued flight now).`
  ...
  registerTool('start_flight', {
    ...
    inputSchema: {
      ...
      stage_producer: z.enum(['internal', 'external']).optional().describe('WHO executes the hand-off-capable thinking stages (Parallel setup is intentionally excluded: once the Report exists, Canary owns its persistent background workflow so this client can surface the Report and end its turn): scout (survey the repos, draft the feature config), docs (gather/infer requirement docs and optionally return a structured requirements draft in the same answer), prd-summary (validate that draft, or distill the docs in a separate hand-off when no valid draft exists), specs-coverage (author the spec files, then map changed tests or newly scoped requirements; unchanged examined pairs can skip the mapping hand-off), run/heal (the run starts in external-heal mode UNCLAIMED and the flight parks: claim_heal with your own session, loop wait_for_heal_task, fix APP code — never tests — signal_run, then submit once the run is terminal; Canary reads the verdict from the run record), and evaluation-export when the mode is localized (the DEFAULT for an external flight — the rewrite is thinking; raw stays internal and deterministic). "external" is the DEFAULT for an MCP caller — you are an interactive agent, so the thinking is yours unless you say otherwise. Pass "internal" to have Canary spawn the CLI named by `agent` on the server instead (the right choice when this client has no file tools, or the user wants a hands-off flight); a single stage can also be handed back with choice:"run-internally" on its checkpoint, so a whole-flight "internal" is rarely what you want. "external" means YOU do those steps in THIS client: each one parks the flight on an `external-work` checkpoint whose data carries the rendered prompt — do the work with your own tools and subagents, then release it with respond_flight_checkpoint(choice:"submit", data:<result>). Canary still validates every result the same way it validates its own agent\'s (the config must parse, the doc must exist on disk, the requirements are reconciled and re-read from the written summary, the specs must compile, the mapping must account for every roster test and the tags are written by Canary\'s own tag-writer before the ledger is recomputed, the run verdict is its own terminal manifest), so the verdict stays evidence rather than your report. Answer choice:"run-internally" on any of those checkpoints to hand that one step back to Canary\'s local agent. STICKY per record for the same reason `agent` is. Pick this when you want the work done with your own context and subagents; the server-owned stages (similarity, scaffold, env, the run\'s own Playwright execution, Parallel setup and Robustness Lab) are unaffected.'),
      session_id: z.string().optional().describe('Stable id for this Claude/Codex conversation. Pass the client\'s real session id when available; otherwise use one stable tracking id for the whole conversation.'),
      client_kind: clientKindInput,
      conversation_name: z.string().optional().describe('Human label for this external agent session.'),
      external_session_url: z.string().optional().describe('Optional URL/deep link that opens this exact conversation. Omit when only generic app launch is possible.'),
      ...
      from_stage: z.string().optional().describe('Start at this stage instead of stage 1 (e.g. "specs-coverage", "run"). Prerequisite artifacts are checked; rejected with the missing one named. Resets this step and the artifacts that depend on it. Tests & coverage keeps Parallel setup; Parallel setup resets itself and Robustness Lab while keeping the run and Report; Robustness Lab resets only itself. Completed Report downloads stay in history.'),
  ...
  }, async ({ repoPaths, description, feature, env, coverage_target, base, yolo, autopilot, agent, stage_producer, session_id, client_kind, conversation_name, external_session_url, fresh, redo, from_stage, feedback }) => {
    ...
    if (latest && latest.status === 'paused' && !fresh && !redo && !from_stage) {
      const resumed = await deps.flightsRequest({
        method: 'POST',
        url: `/api/flights/${encodeURIComponent(latest.flightId)}/resume`,
        payload: externalAgentSession ? { externalAgentSession } : undefined,
      })
    ...
    if (started.statusCode === 409 && startedBody.type === 'flight_exists_requires_choice') {
      return asJsonResult({
        ...
        next: 'This feature already has a flight record. Re-call start_flight with redo:true to restart from stage 1, or from_stage:"<stage>" to jump (prerequisites checked) — OMIT repoPaths/description so the frozen stored values are reused. Redo resets all stages; from_stage resets only affected artifacts. Tests & coverage keeps Parallel setup, Parallel setup resets itself and Robustness Lab, and Robustness Lab keeps the completed Report. Completed Report downloads stay in history. A paused record resumes automatically without either flag — resume never wipes.',
      })
    }
