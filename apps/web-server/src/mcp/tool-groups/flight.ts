  ...
  const flightNext = (view: Record<string, unknown>): string => {
    ...
    if (view.status === 'waiting-for-approval') {
      ...
      if (cp?.kind === 'portify-gate') {
        return `${reportReady ? `${reportReady} ` : ''}${base} This is the final Parallel setup ask, after the Report but before any Portify agent or double-boot cost: "run" starts the server-owned background workflow (agent edits port wiring in a throwaway worktree, concurrent double-boot verifies — heavy stacks can take 30-60+ min; a sibling feature's saved overlay for the same app is reused and verified first, so the agent may not run at all); "skip" keeps the feature serial (runs go one at a time) and the flight continues — a later flight can ask again. The Report is independent of this choice and remains reviewable.`
      }
    ...
    if (view.status === 'running') {
      if (reportReady) {
        return `${reportReady} Tell the user now, then end your turn. Final Parallel setup is continuing as Canary-owned persistent background work. Do not keep polling merely to hold this conversation open; the Flight page stays live, and re-check only when the user asks.`
      }
