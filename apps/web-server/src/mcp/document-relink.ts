...
export async function requestBrokenDocumentPath(options: {
  ...
  return requestUserInput(request, ctx.clientFacts(), {
    ...
    fallback: () => asJsonResult({ status: 'needs-input', reason: 'elicitation-unavailable', feature, brokenDoc: doc, ...(url ? { url } : {}),
      next: `Ask the user to repair ${doc.relPath} with Relink in Canary Lab, then retry ${command} with the same arguments. Do not omit the missing source or create a recovery copy.` }),
  }, async ({ local_path }) => {
