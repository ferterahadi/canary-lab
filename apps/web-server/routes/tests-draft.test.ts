...
import type { AgentSessionRef } from '../lib/agent-session-log'
...
vi.mock('../lib/draft-agent-session', async () => {
  const actual = await vi.importActual<typeof import('../lib/draft-agent-session')>('../lib/draft-agent-session')
  return {
    ...actual,
...
vi.mock('../lib/draft-store', async () => {
  const actual = await vi.importActual<typeof import('../lib/draft-store')>('../lib/draft-store')
  return {
    ...actual,
...
vi.mock('../lib/draft-file-resolver', async () => {
  const actual = await vi.importActual<typeof import('../lib/draft-file-resolver')>('../lib/draft-file-resolver')
  return {
    ...actual,
...
async function seedPlanningDraft(deps: TestsDraftRouteDeps): Promise<string> {
  ...
  const { createDraft, transition } = await import('../lib/draft-store')
...
describe('pipeline-driver guard branches', () => {
  ...
  it('runPlanStage patchDraft bails when the draft is deleted mid-flight', async () => {
    ...
    const { createDraft, transition, deleteDraft } = await import('../lib/draft-store')
    const deps = makeDeps({
