...
function bodyCopy(status: PortifyStatus, agent: string): string {
  switch (status) {
    ...
    case 'editing':
      return `${agent} is rewriting the listeners to read injected ports in the worktree(s) above (and declaring the matching \`ports\` slots in the suite configuration). The live work happens in your ${agent} window — Canary Lab has no local transcript. When the edits are done, ${agent} submits them and Canary Lab boots the stack twice to verify.`
    case 'verifying':
    ...
    case 'ready-to-save':
      return `Verified — booted twice on disjoint ports, both healthy. Review the diff and Save to capture it as the suite's ephemeral overlay (nothing is committed or merged).`
    case 'saved':
      return `Saved as the suite's ephemeral overlay. It now applies into a fresh per-run worktree before every boot, so the suite runs concurrently without a port clash.`
    case 'failed':
