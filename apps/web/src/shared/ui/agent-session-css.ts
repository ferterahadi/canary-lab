...
.agentts-row::before{content:'';position:absolute;left:6.25px;top:16px;bottom:-1px;width:1.5px;background:linear-gradient(180deg,var(--border-default),color-mix(in srgb,var(--border-default) 25%,transparent));border-radius:2px}
.agentts-row:last-child::before{display:none}
.agentts-node{position:absolute;left:0;top:2px;width:14px;height:14px;border-radius:50%;border:1.5px solid var(--border-default);display:grid;place-items:center;background:var(--bg-base);z-index:1}
/* A tool row folds its caption into its box, so its first line of text starts
   one margin + one border + one padding lower than a prose row's. Nudge the
   marker and its connector down to match — the node must sit on the line it
   marks. Re-derive these two if the box's margin or padding changes. */
.agentts-row[data-kind="tool-call"]>.agentts-node,.agentts-row[data-kind="tool-result"]>.agentts-node{top:8px}
.agentts-row[data-kind="tool-call"]::before,.agentts-row[data-kind="tool-result"]::before{top:22px}
.agentts-sysrow{position:relative;margin:0;padding:0 0 8px 23px;list-style:none;animation:agentts-in .26s cubic-bezier(.22,1,.36,1) both}
.agentts-sysrow:last-child{padding-bottom:2px}
.agentts-sysrow::before{content:'';position:absolute;left:6.25px;top:16px;bottom:-1px;width:1.5px;background:linear-gradient(180deg,var(--border-default),color-mix(in srgb,var(--border-default) 25%,transparent));border-radius:2px}
.agentts-sysrow:last-child::before{display:none}
.agentts-sysnode{position:absolute;left:0;top:2px;width:14px;height:14px;border-radius:4px;border:1.5px solid var(--border-default);display:grid;place-items:center;background:var(--bg-base);color:var(--text-muted);z-index:1}
.agentts-sysnode svg{width:8.5px;height:8.5px}
...
.agentts-toolbtn{display:flex;width:100%;align-items:center;gap:10px;padding:5px 10px;background:none;border:none;cursor:pointer;text-align:left;min-width:0}
...
/* The clock takes the free space, so it right-aligns beside the chevron on a
   short row and simply follows the text on a row long enough to truncate. The
   padding is its minimum separation from a path that ran the full width. */
.agentts-toolbtn .agentts-time{flex:none;margin-left:auto;padding-left:14px}
.agentts-toolbtn .agentts-chev{margin-left:0}
.agentts-errtag{flex:none;font-family:var(--font-mono);font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:var(--danger)}
.agentts-chev{margin-left:auto;color:var(--text-muted)}
.agentts-pre{margin:0;border-top:1px solid var(--border-default);padding:9px 11px;font-family:var(--font-mono);font-size:11px;line-height:1.55;color:var(--text-secondary);background:var(--bg-base);white-space:pre-wrap;word-break:break-word;max-height:300px;overflow:auto}
.agentts-sub{border-top:1px solid var(--border-default)}
.agentts-subbtn{display:flex;width:100%;align-items:center;gap:9px;padding:5px 10px;background:none;border:none;cursor:pointer;text-align:left;min-width:0}
...
.agentts-nest{margin:0 0 0 14px;padding:8px 11px 4px 11px;list-style:none;border-left:2px solid var(--border-default);background:var(--bg-base)}
.agentts-nestrow{padding-bottom:8px}
.agentts-nestrow::before{left:5px;top:14px}
.agentts-nestrow .agentts-node{width:11px;height:11px;top:3px}
.agentts-nestrow[data-kind="tool-call"]>.agentts-node,.agentts-nestrow[data-kind="tool-result"]>.agentts-node{top:10px}
.agentts-nestrow[data-kind="tool-call"]::before,.agentts-nestrow[data-kind="tool-result"]::before{top:21px}
...
.agentts-working{position:relative;display:flex;align-items:center;gap:7px;min-height:17px;padding:2px 0 0 23px;color:var(--running);list-style:none}
...
.agentts-working::before{content:'';position:absolute;left:6.25px;top:-13px;width:1.5px;height:17px;border-radius:2px;background:linear-gradient(180deg,transparent,color-mix(in srgb,var(--running) 60%,transparent))}
...
.agentts-worknode{position:absolute;left:0;top:2px;box-sizing:border-box;width:14px;height:14px;border:1.5px solid color-mix(in srgb,var(--running) 40%,var(--border-default));border-radius:50%;background:var(--bg-base);box-shadow:0 0 0 3px color-mix(in srgb,var(--running) 7%,transparent);z-index:1}
