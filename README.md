...
Starting from **v1.1.0**, Canary Lab is built for an **agent-first workflow**. Codex, Claude, or another coding agent can write tests, start or claim runs, read failure context, make fixes, and signal the next runner action.

Canary Lab stays the **local run monitor** and **source of truth** for evidence. The coding agent remains the main place where instructions happen.
...
Canary Lab has a narrow boundary. It does not define a new test language, assertion model, or browser runner. Engineers and agents write normal Playwright tests, and Playwright executes them.
...
- Optional repair agents: `claude` or `codex` on `PATH`.
...
1. **Features**: discovered `features/<name>/` folders and run buttons.
...
The UI also has an Add Test flow. It can turn a PRD or uploaded document into a generated plan and Playwright files for review. The generated tests still run through Playwright.
...
Envsets are temporary environment files for a feature. During a run, Canary Lab backs up the target files, applies the selected envset, then restores the originals when the run ends.

Use the Envsets tab to create an env, add files, edit values, and start a run. Envsets are stored under:

```text
features/<feature>/envsets/
```

Feature configs can also make service startup env-specific. For example, a `local` env can start services, while a `production` env can skip local startup and point tests at a deployed URL.

External clients should use the installed Canary Lab agent skill and MCP author tools for env capture. The old project-local Claude/Codex env-import skill files are no longer scaffolded.
...
Completed runs can export an Evaluation Report from the run detail Overview tab. The download is a `.zip` with `evaluation.html` and captured videos.
...
Auto-heal starts a Claude or Codex agent in a PTY tab when a run fails. Canary Lab renders `apps/web-server/prompts/heal-agent.md` with the active run paths and passes it to the agent.
...
External Claude/Codex clients can use the MCP `author` profile to create durable Canary Lab tasks without asking Canary Lab to author content:
...
The UI shows these tasks as generated by an external client. It stores stages, session names, and downloadable artifacts, but it does not show or replay the external client transcript.
...
- Envset runs overwrite target files while active. If the process is killed during backup or restore, reopen the UI and use envset controls to recover.
