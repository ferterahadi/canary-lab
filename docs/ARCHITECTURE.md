...
| Heal workspace trust ↔ the folder-trust prompt | `ensureHealWorkspaceTrusted` (`runs/logic/runtime/run-heal-agent.ts`) ↔ `agent-sessions/logic/agent-workspace-trust.ts` ↔ the `trust-prompt` fingerprint (`runs/logic/runtime/heal-failure-classifier.ts`) ↔ `agentCause` (`shared/run-state.ts`) ↔ `healAgentCauseSuffix` + `HEAL_CAUSE_PHRASE` (`apps/web/.../StageStatusLines.tsx`). The heal REPL is the only agent spawned on an interactive TTY, so it is the only one Claude Code's folder-trust prompt can stop; trust inherits from any ancestor, so one entry on the project root covers every run dir. If the seeding path ever stops running, the classifier is what keeps the stall from reading as "the agent tried and failed". | `agent-workspace-trust.test.ts` + `heal-workspace-trust.test.ts` + the real-tail case in `heal-failure-classifier.test.ts` | `cl_locate-agent-session-logs` |
