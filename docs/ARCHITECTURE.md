...
| `robustness` | Green-run matrix under latency, duplicate-write and restart perturbations |
...
Parallel setup and Robustness Lab. The Lab follows port setup so its proxy can
use the saved slots; neither job delays the first Report.
...
Parallel setup; repeating Parallel setup resets it and the Lab while keeping
the run and Report; repeating the Lab keeps the Report. Completed archives
remain in history, and a new export can include later Lab findings. A full redo
resets every stage.
...
- `external` is available to MCP-started Flights. Scout, docs, PRD summary,
  ...
  submits their results. Parallel setup and Robustness Lab are deliberately
...
Run/heal parks once per engagement while the client drives its standalone
workflow. Portify retains a legacy checkpoint consumer only to migrate Flights
persisted before Parallel setup became server-owned.

The browser can request a cooperative takeover of the current handoff. The
request is persisted as `checkpoint.data.takeoverRequestedAt` and broadcast by
the Flight store, but the conductor stays parked. `get_flight` tells the external
client to stop and acknowledge with `run-internally`; any `submit` after the
request receives the typed `flight_takeover_requested` rejection. A confirmed
force uses the same `run-internally` transition when the client is gone. Because
checkpoint transitions and store writes are synchronous, request versus submit
is serialized by the server event loop: submit-first settles normally;
request-first closes submission before Canary can start locally.
...
| **Behavior certificate ↔ offline checker** | `buildBehaviorCertificate` (`evaluation/logic/behavior-certificate.ts`) reads the suite with `hashFeatureSpecs` (`runs/logic/dirty-specs/detect.ts` — `.spec.ts` files directly under `e2e/`, nothing deeper) and digests it with `digestOfSpecHashes` (`runs/logic/runtime/run-suite-snapshot.ts`) ↔ the zero-dependency `apps/web-server/assets/verify-certificate.mjs` (copied to `dist/` by `tools/prepare-assets.mjs`, zipped beside `certificate.json`) re-derives both from files on disk with its own copies of that lister and that digest ↔ the wire types in `shared/verification-strength/certificate.ts` (`@2` adds the optional `robustness` block, re-stated from the newest settled Robustness Lab job built from the run — `evaluation-export-archive.ts` looks it up in `runs/logic/robustness/store.ts`; the checker reads `@1` and `@2` and relays the block without re-judging it) ↔ the `certificate` digest on `get_evaluation_export` / the full file on `download_evaluation_export` (`mcp/tool-groups/authoring-export.ts`) ↔ `mcp-export-instructions.md` + every shipped `canary-lab-export/SKILL.md`. The checker is the point: a third party must reproduce the digest without Canary Lab code, so the lister and the digest line format cannot change on one side only. The certificate text names what it does not prove (the absence of weakening first) — never soften it. | `behavior-certificate.test.ts` runs the real checker (`spawnSync`) against certificates built from real dirs, incl. a tampered spec, a shifted assertion and a missing `--suite`; `authoring-export.test.ts` pins the digest shape | `cl_run-evidence-invariants` |
| **Robustness Lab over every surface** | the robustness routes (`runs/routes/robustness.ts`) are the one admission judge — green run, declared port slots, valid envelope, one matrix per suite — for the flight stage (`flights/logic/stages/robustness.ts`), the web pane (`apps/web/src/shared/api/robustness.ts`) and the MCP tools `start_robustness` / `get_robustness` (`mcp/tool-groups/robustness.ts`, thin over `robustnessRequest` = `app.inject`) ↔ the job record shape in `shared/robustness/jobs.ts` ↔ `start_run`'s `perturbation` input and the heal context's `perturbation` + rule (`runs/logic/heal/external-heal-surface.ts`) ↔ the Robustness Lab sections of `mcp-repair-instructions.md`, `mcp-flight-instructions.md` and every shipped `canary-lab-run/SKILL.md` + `canary-lab/SKILL.md`. A skipped cell and an unconfirmed finding are never passes, on any surface. | `robustness.test.ts` (routes, stage, MCP group), `server.smoke.test.ts` profile mirrors, `apps/cli/agent.test.ts` skill pins | `cl_add-mcp-tool` / `cl_sync-agent-surfaces` |
