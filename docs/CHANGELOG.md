...
- **[General]** **Flight is now the front door — the whole experience rethought around 7 user-facing steps** (Repo scan → Suite setup → Requirements → Test authoring & coverage → Parallel readiness → Test Run → Evaluation Report).
  ...
  - **Requirements gets its own stop — but not under autopilot.** The stage always offers the fork: add files yourself, or link a local PRD by path (symlinked, so your original stays the live source; broken links are surfaced, never crash the list); paths mentioned in your intent (e.g. `~/Documents/prd.md`) are linked in automatically. With autopilot on (the default) the fork releases itself — `continue` when docs are already there, and otherwise the collector agent gathers them from the repos guided by your intent. It only reaches you if that collector comes back empty-handed, or if you turn autopilot off to be asked every time.
  ...
  - **Pause, stop, start over — always in reach.** A new user pause parks the flight resumably and stops everything it started — the stage's agent, the test run, and any repair in progress — so Continue re-runs the step from the top; Stop aborts; Start over redoes the same record from step 1. Stopping only the run stays on the Test Run step (**Stop run**, **Cancel repair**), which lets the flight carry on and ask what to do next. Intent and repos are editable on the Repo Scan stage while the flight is paused/stopped — a repo change restarts from Repo Scan by design.
  ...
  - **Autopilot is a live switch, and a step you re-run always asks.** The flight header's facts strip now carries an **Autopilot on/off** toggle you can flip at any time, in any flight state — it takes effect at the next checkpoint (a checkpoint the flight is already parked on stays yours to answer). And picking Continue → from a step now **always parks that step's first checkpoint**, even with autopilot on: choosing to re-run Requirements is the intent to answer it differently, so the safe default no longer re-decides it within the same tick and makes a deliberate redo look like a plain resume.
  - **Continue names the step it will actually resume.** A merged step whose second half failed (e.g. Requirements, when the summary distiller fails after the docs were approved) used to read as finished, so the menu advertised the NEXT step ("Resume at Test authoring & coverage") while resume in fact re-entered Requirements. It now names the step that owns the first unfinished half. Resume still continues from the last state; resetting a step to zero is what "From a step…" does.
